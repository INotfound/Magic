...
namespace Magic{
    ...
    MultiPartParser::MultiPartParser(const StringView& sv){
        lookbehind = nullptr;
        resetCallbacks();
        setBoundary(sv);
    }
    ...
    void MultiPartParser::setBoundary(const StringView& sv){
        reset();
        this->boundary = StringCat("\r\n--",sv);
        boundaryData = this->boundary
                           .data();
        boundarySize = this->boundary
                           .size();
        indexBoundary();
    ...
    void MultiPartParser::processPartData(size_t& prevIndex,size_t& index,const char* buffer,size_t len,size_t boundaryEnd,size_t& i,char c,State& state,int& flags){
        prevIndex = index;
