...
const char* LogLevel::ToString(LogLevel::Level const level){
    switch (level)
    {
    #define LEVEL(Name)\
        case LogLevel::Name: \
...
Logger::Logger(const std::string& name) 
    :m_LogName(name),m_Level(LogLevel::DEBUG){
}
...
LogWrap::LogWrap(Ptr<Logger>& logger,const LogLevel::Level level,Ptr<LogEvent>&& event):m_Logger(logger),m_Level(level),m_Event(std::move(event)){
}
